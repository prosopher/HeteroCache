...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Train and evaluate a translated layer window anchored at a chosen reference-target layer position by replaying target prefill below the window, injecting the translated window, and continuing above it. Source/target KV caches can be rotated into SVD principal coordinates before translation and inverse-rotated afterward. SQuAD v1.1 is evaluated with direct context-span selection, and task decomposition, logit-KL comparison, and synergy analysis are run automatically as part of the same execution."
    )
